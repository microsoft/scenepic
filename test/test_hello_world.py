...
def create_image_mesh(scene: sp.Scene, filename: str, layer_id: str):
    """Utility function to create an image mesh.

    Args:
        scene (sp.Scene): current Scene
...
def test_hello_world(asset):
    ...
    hand_mesh = sp.load_obj(asset("hand.obj"))
    cmvs, cmts = sp.LoopSubdivStencil(hand_mesh.triangles, 1, True).apply(hand_mesh.positions)

    model_mesh_lo = scene.create_mesh(shared_color=sp.Color(1.0, 0.0, 0.0))
    model_mesh_lo.add_mesh(hand_mesh, reverse_triangle_order=True)
    model_mesh_hi = scene.create_mesh(shared_color=sp.Color(0.0, 0.0, 1.0))
    ...
    hand_camera = sp.Camera(center=hand_center + np.array([0.0, 0.0, 0.5]),
                             look_at=hand_center,
                             up_dir=np.array([0.0, 1.0, 0.0]),
                             fov_y_degrees=45.0)
    hand_shading = sp.Shading(bg_color=sp.Colors.White)
    hand_ui_parameters = sp.UIParameters()
    ...
    for i in range(NUM_FRAMES):
        ...
        frame_hello.add_mesh(mesh_hello,
                             transform=sp.Transforms.Translate([math.cos(angle),
                                                                 math.sin(angle),
                                                                 0]))
        frame_hello.add_mesh(mesh_analog,
                             transform=sp.Transforms.Translate([math.cos(angle - 0.5 * math.pi),
                                                                 math.sin(angle - 0.5 * math.pi),
                                                                 -0.5]))
        frame_hello.add_mesh(mesh_science,
                             transform=sp.Transforms.Translate([math.cos(angle + math.pi),
                                                                 math.sin(angle + math.pi),
                                                                 -1.0]))

        mesh_primitives = scene.create_mesh(layer_id="Primitives")
        mesh_primitives.add_sphere(color=sp.Color(0, 1, 0),
                                   transform=sp.Transforms.Scale(0.2 * (2 + math.cos(angle))),
                                   fill_triangles=False,
                                   add_wireframe=True)
        mesh_primitives.add_cube(color=sp.Color(0, 0, 1),
                                 transform=sp.Transforms.Translate([-1.0, -1.0, -3.0]))
        frame_hello.add_mesh(mesh_primitives)

        mesh_noise = scene.create_mesh(shared_color=sp.Color(1.0, 0.0, 0.0), layer_id="Noise")
        mesh_noise.add_cylinder()
        mesh_noise.apply_transform(sp.Transforms.Scale([0.02, 0.1, 0.1]))
        mesh_noise.apply_transform(
            sp.Transforms.RotationToRotateXAxisToAlignWithAxis(np.array([0.5, 0.5, 0.5])))
        ...
        frame_hand.add_mesh(model_mesh_lo,
                            transform=sp.Transforms.RotationAboutZAxis(math.cos(angle)))
        frame_hand.add_mesh(model_mesh_hi,
                            transform=sp.Transforms.RotationAboutZAxis(math.cos(angle + math.pi)))
