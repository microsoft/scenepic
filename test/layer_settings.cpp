...
int test_layer_settings()
{
    ...
    for(auto i=0; i<2; ++i)
    {
        ...
        frame2d->add_circle(0, 0, 2, sp::Colors::Black, 1.0f, sp::Color::None(), "one");
        frame2d->add_circle(0, 0, 3, sp::Colors::Black, 1.0f, sp::Color::None(), "two");

        frame2d->add_text("null", 0, 0);
        frame2d->add_text("one", 100, 100, sp::Colors::White, 12.0f, "sans-serif", "one");
        frame2d->add_text("two", 200, 200, sp::Colors::White, 12.0f, "sans-serif", "two");

        sp::CoordinateBuffer coords(2, 2);
        coords << 0, 0,
                  0, 0;
        frame2d->add_line(coords);
        frame2d->add_line(coords.array() + 1, sp::Colors::Black, 1.0f, sp::Color::None(), false, "one");
        frame2d->add_line(coords.array() + 2, sp::Colors::Black, 1.0f, sp::Color::None(), false, "two");

        frame2d->add_rectangle(0, 0, 0, 0);
        frame2d->add_rectangle(0, 0, 10, 10, sp::Colors::Black, 1.0f, sp::Color::None(), "one");
        frame2d->add_rectangle(0, 0, 20, 20, sp::Colors::Black, 1.0f, sp::Color::None(), "two");
