...
void assert_near(float actual, float expected, int &result, const std::string& tag, float epsilon)
{
    if (std::abs(actual - expected) > epsilon)
    {
        result = EXIT_FAILURE;
        std::cerr << tag << " is incorrect: " << actual << " != " << expected << std::endl;
    }
}
...
void assert_buffer_equal(const svt::Json::Value &actual, const svt::Json::Value &expected, int &result, const std::string &tag, float epsilon)
{
    ...
    assert_equal(actual_matrix.rows(), expected_matrix.rows(), result, tag + " rows");
    assert_equal(actual_matrix.cols(), expected_matrix.cols(), result, tag + " cols");
    
    for(auto row=0; row < actual_matrix.rows(); ++row)
    {
        for(auto col=0; col < actual_matrix.cols(); ++col)
        {
            assert_near(actual_matrix(row, col),
...
void assert_quantized_buffer_equal(const svt::Json::Value &actual, const svt::Json::Value &expected, int &result, const std::string &tag, float epsilon)
{
    ...
    assert_equal(actual_values.size(), expected_values.size(), result, tag + " size");
    if (result == EXIT_FAILURE)
    {
        return;
    }
    ...
    {
        assert_near(actual_values[i], expected_values[i], result, tag + "[" + std::to_string(i) + "]", epsilon);
        if (result == EXIT_FAILURE)
        {
            std::cerr << tag << " is incorrect: " << std::endl
                      << actual << std::endl
...
void assert_json_equal(const svt::Json::Value &actual, const svt::Json::Value &expected, int &result, const std::string &tag, float epsilon)
{
    ...
    {
        result = EXIT_FAILURE;
        std::cerr << "type mismatch: " << actual.type() << " != " << expected.type() << std::endl;
        return;
    }
    ...
    if (actual.type() == svt::Json::ValueType::objectValue)
    {
        ...
        {
            assert_equal(actual_members[i], expected_members[i], result, tag + " member");
            const std::string &member = actual_members[i];
            if (member == "VertexBuffer" || member == "Transform" || member == "CoordinateBuffer" ||
                member == "CircleBuffer" || member == "InstanceBuffer" || member == "ValueBuffer" ||
                member == "LayerBuffer" || member == "Projection" )
            {
                assert_buffer_equal(actual[member], expected[member], result, tag + "." + member, epsilon);
            }
            else if(member == "QuantizedBuffer")
            {
                assert_quantized_buffer_equal(actual, expected, result, tag + ".QuantizedBuffer", epsilon);
            }
            else
            {
                assert_json_equal(actual[member], expected[member], result, tag + "." + member, epsilon);
            }
        }
    }
