...
template <typename T>
void assert_equal(const std::vector<T> &actual, const std::vector<T> &expected, int &result, const std::string &tag)
{
    ...
    if (result == EXIT_SUCCESS)
    {
        ...
        {
            assert_equal(actual[i], expected[i], result, tag + "[" + std::to_string(i) + "]");
...
template <typename T>
void assert_lessthan(const T &actual, const T &expected, int &result, const std::string &tag)
{
    if (expected < actual)
    {
        result = EXIT_FAILURE;
...
void assert_allclose(const Eigen::MatrixBase<Derived>& actual, const Eigen::MatrixBase<Derived> &expected, int &result, const std::string &tag, typename Derived::Scalar tolerance=1e-6)
{
    typename Derived::Scalar max_diff = (actual - expected).array().abs().maxCoeff();
    if(max_diff > tolerance)
    {
        result = EXIT_FAILURE;
        std::cerr << tag << " is incorrect, maximum difference is " << max_diff << " which is not within tolerance of " << tolerance << std::endl;
