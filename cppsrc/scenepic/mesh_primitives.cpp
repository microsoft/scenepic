...
void Mesh::add_cube(const Color &color,
                    ...
                    bool add_wireframe)
{
    ...
    m.add_quad(color, p4, p6, p2, p0, VectorNone(), fill_triangles, add_wireframe);
    m.add_quad(color, p2, p6, p7, p3, VectorNone(), fill_triangles, add_wireframe);
    m.add_quad(color, p4, p0, p1, p5, VectorNone(), fill_triangles, add_wireframe);
...
void Mesh::add_thickline(const Color &color,
...
void Mesh::add_cone(const Color &color,
                    const Transform &transform,
...
void Mesh::add_coordinate_axes(float length,
                               float thickness,
                               const Transform &transform)
{
    this->check_instances();
...
void Mesh::add_camera_frustum(const Color &color,
                              float fov_y_degrees,
                              float aspect_ratio,
                              float depth,
                              float thickness,
                              const Transform &transform)
{
    ...
    m.add_thickline(color, {0, 0, 0}, {+width, +height, depth}, 0.4f * thickness, thickness);
    m.add_thickline(color, {0, 0, 0}, {+width, -height, depth}, 0.4f * thickness, thickness);
    m.add_thickline(color, {0, 0, 0}, {-width, -height, depth}, 0.4f * thickness, thickness);
    m.add_thickline(color, {0.0, 0.0, 0.0}, {-width, +height, depth}, 0.4f * thickness, thickness);
    m.add_thickline(color, {+width, +height, depth}, {-width, +height, depth}, thickness, thickness);
    m.add_thickline(color, {-width, +height, depth}, {-width, -height, depth}, thickness, thickness);
    m.add_thickline(color, {-width, -height, depth}, {+width, -height, depth}, thickness, thickness);
    m.add_thickline(color, {+width, -height, depth}, {+width, +height, depth}, thickness, thickness);
...
void Mesh::add_disc(const Color &color,
                    const Transform &transform,
                    ...
                    bool add_wireframe)
{
    ...
    VectorBuffer vertices = VectorBuffer::Zero(static_cast<Eigen::Index>(segment_count) + 1, 3);
    ...
    VectorBuffer normals = VectorBuffer::Zero(static_cast<Eigen::Index>(segment_count) + 1, 3);
    normals.col(0).fill(1.0f);

    this->add_mesh_with_normals(vertices, normals, triangles, colors, UVBufferNone(), transform, false, fill_triangles, add_wireframe);
}

void Mesh::add_cylinder(const Color &color,
                        const Transform &transform,
                        std::uint32_t segment_count,
                        bool fill_triangles,
                        bool add_wireframe)
{
    ...
    // Note, we do not re-use disc vertices for the barrel, as this gives bad normals
...
void Mesh::add_sphere(const Color &color,
                      const Transform &transform,
                      bool fill_triangles,
                      bool add_wireframe)
{
...
void Mesh::add_icosphere(const Color &color,
                         const Transform &transform,
                         std::uint32_t steps,
                         bool fill_triangles,
                         bool add_wireframe)
{
    ...
    for (std::uint32_t iter = 0; iter < steps; ++iter)
    {
        ...
        for (auto triangle = 0; triangle < triangles.rows(); ++triangle)
        {
            ...
            for (const auto &edge : edges)
            {
                ...
                {
                    Vector v = vertex_positions.row(edge.first) + vertex_positions.row(edge.second);
                    v.normalize();
                    v *= radius;
                    v_idx[edge] = new_e_vs[edge] = static_cast<std::uint32_t>(vertex_positions.rows());
                    append_row(vertex_positions, v);
                }
            }

            // Append new triangles
            append_row(new_triangles, Triangle(a, v_idx[Edge(a, b)], v_idx[Edge(a, c)]));
            append_row(new_triangles, Triangle(v_idx[Edge(a, b)], v_idx[Edge(b, c)], v_idx[Edge(a, c)]));
            append_row(new_triangles, Triangle(v_idx[Edge(a, c)], v_idx[Edge(b, c)], c));
            append_row(new_triangles, Triangle(v_idx[Edge(a, b)], b, v_idx[Edge(b, c)]));
        }
    ...
    if (!m_texture_id.empty())
    {
        ...
        {
            uvs(v_idx, 0) = static_cast<float>(0.5f - 0.5f * std::atan2(vertex_positions(v_idx, 2), vertex_positions(v_idx, 0)) / PI); // Azimuth
            uvs(v_idx, 1) = static_cast<float>(1.0f - std::acos(vertex_positions(v_idx, 1) * 2.0f) / PI); // Inclination
        }

        // Duplicate vertices across the longitude seam - not particularly efficient and bit of a hack but works ok.  Results in non watertight mesh.
        // Loop over triangles
        TriangleBuffer new_triangles;
        for (auto triangle = 0; triangle < triangles.rows(); ++triangle)
        {
            ...
            // Rotate triangle indices to ensure a is east most vertex, preserving triangle winding order
            ...
            if (a_isEast && c_isWest) // Duplicate c vertex
            {
                ...
                c = static_cast<uint32_t>(vertex_positions.rows() - 1); // Will overwrite c in triangle
            }
            if (a_isEast && b_isWest) // Duplicate b vertex
            {
                ...
                b = static_cast<uint32_t>(vertex_positions.rows() - 1); // Will overwrite b in triangle
            }
    ...
    this->add_mesh_with_normals(std::move(vertex_positions),
...
void Mesh::add_uv_sphere(const Color &color,
                        const Transform &transform,
                        std::uint32_t lat_count,
                        std::uint32_t long_count,
                        bool fill_triangles,
