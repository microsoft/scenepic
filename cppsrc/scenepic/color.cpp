...
{
    return Color(-std::numeric_limits<Color::Scalar>::infinity(),
                 -std::numeric_limits<Color::Scalar>::infinity(),
                 -std::numeric_limits<Color::Scalar>::infinity());
}

Color::Color(const Color::Scalar &red, const Color::Scalar &green, const Color::Scalar &blue) : Base(red, green, blue)
{    
    assert((red >= 0.0 && red <= 1.0) || red == -std::numeric_limits<Color::Scalar>::infinity());
    assert((green >= 0.0 && green <= 1.0) || green == -std::numeric_limits<Color::Scalar>::infinity());
    assert((blue >= 0.0 && blue <= 1.0) || blue == -std::numeric_limits<Color::Scalar>::infinity());
}
