// ----------------------------------------------------------------------------
//
// transforms.h -- Simple class which encodes color values
//
 ...
 * aligns with the provided axis.  NB one degree of freedom is underconstrained.
 * \param axis the axis to which to align the x-axis
 * \return a quaternion
 */
...
/** Creates a WebGL projection transform (i.e. that goes from view coordinates to NDC).
 *  \param fov_y_degrees the field of view of the camera in degrees
 *  \param aspect_ratio the aspect ratio of the camera
 *  \param znear the z value of the near clipping plane of the frustum
 *  \param zfar the z value of the far clipping plane of the frustum
 *  \return a WebGL projection matrix
 */
Transform gl_projection(double fov_y_degrees, double aspect_ratio, double znear, double zfar);

/** Creates a WebGL projection transform (i.e. that goes from view coordinates to NDC) from
 *  an OpenCV-style camera matrix.
 *  \param camera_matrix the OpenCV-style camera matrix
...
/** Creates a WebGL view transform (i.e. that goes from world coordinates to camera view coordinates)
 *  from an OpenCV-style extrinsic matrix
 *  \param extrinsic the camera-to-world matrix, which places the camera in the scene
 *  \return a WebGL view matrix
 */
