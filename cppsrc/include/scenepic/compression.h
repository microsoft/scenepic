...
namespace scenepic
{
    ...
    {
        const std::uint8_t *source_buf = reinterpret_cast<const std::uint8_t *>(matrix.data());
        ...
        std::uint32_t *rows_ptr = reinterpret_cast<std::uint32_t *>(deflate_bytes.data() + dest_len);
    ...
    {
        const std::uint32_t *rows_ptr = reinterpret_cast<const std::uint32_t *>(buffer.data() + buffer.size() - 5);
        Eigen::Index rows = *rows_ptr;
        Eigen::Index cols = *buffer.rbegin();
        auto inflate_bytes = inflate(buffer.data(), buffer.size() - 8, rows * cols * sizeof(typename Derived::Scalar));
        Eigen::Map<Derived> matrix(reinterpret_cast<typename Derived::Scalar *>(inflate_bytes.data()), rows, cols);
