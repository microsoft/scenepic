...
public:
     ...
     *  \param frame_id a unique identifier for the Frame3D (will be automatically populated if not provided).
     *  \param focus_point optional 3D focus point for this frame (with optional 3D rotation for orientation lock), used in the ScenePic user interface.
     *  \param mesh_ids optionally pre-populate this Frame3D with a set of Meshes, for convenience.
     *  \param camera an optional camera for the frame
     *  \return a new Frame3D object
     */
    std::shared_ptr<Frame3D> create_frame(const std::string &frame_id = "",
    ...
    /** Creates a new Frame3D object and appends to the list of Frame3Ds in the Canvas3D.
     *  \param frame_id a unique identifier for the Frame3D (will be automatically populated if not provided).
     *  \param meshes optionally pre-populate this Frame3D with a set of Meshes, for convenience.
     *  \param focus_point optional 3D focus point for this frame (with optional 3D rotation for orientation lock), used in the ScenePic user interface.
    ...
    std::shared_ptr<Frame3D> create_frame_with_meshes(const std::vector<std::shared_ptr<MeshType>> &meshes,
     ...
     *  Each Mesh object can optionally be part of a user-identified layer (see Mesh constructor).
     *  Calling set_layer_settings will result in an additional drop-down selector in the ScenePic user interface.
     *  Currently, opacity is only guaranteed to be correct for Meshes that do not overlap.
     *  If you know one mesh should be rendered before another for opacity purposes, use the renderOrder setting:
     *  low numbers will be drawn before high numbers, and layers with no renderOrder set will be drawn first.
     *  Meshes without layer ids, or with layer ids that are not specified in this dictionary, will be drawn filled,
     *  without wireframe, at opacity 1.0, and before any layers with renderOrder set.
     */
     ...
     *  This file will be used to drive playback, i.e. frames will be displayed in time with
...
private:
     ...
     */
    Canvas3D(const std::string &canvas_id,
             double height,
             double width,
