// ----------------------------------------------------------------------------
//
// scene.h -- Top level container representing an entire SP Scene.
//
...
struct QuantizationInfo
{
    ...
    /** Constructor. */
    QuantizationInfo(std::uint32_t keyframe_count,
...
public:
    /** Constructor.
     *  \param scene_id a unique identifier for the Scene (will be automatically populated if not provided).
     */
    Scene(const std::string &scene_id = "");

    /** Defines a CSS layout grid for the canvases of the ScenePic. Should not be used in conjunction with
     *  `html_id` for individual canvases. See https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Grid_Layout
     *  for more information.
     *  \param width CSS declaration for the width of the ScenePic.
     *  \param grid_template_rows CSS declaration defining the rows of the ScenePic
     *  \param grid_template_cols CSS declaration defining the columns of the ScenePic
     */
     ...
     *  \param canvas_id a unique identifier for the Canvas (will be automatically populated if not provided).
     ...
     *  \param html_id optional id of an HTML element to use as this Canvas's parent in the HTML DOM (otherwise simply appended to document).
     ...
     *  \param media_id optional ID of a media file to attach to the canvas.  This file will be used to drive playback, i.e. frames will be displayed in time with the playback of the media file.
     *  \return a reference to the new Canvas object
     */
    std::shared_ptr<Canvas3D> create_canvas_3d(const std::string &canvas_id = "",
                                               double width = 400,
                                               double height = 400,
     ...
     *  \return a reference to the new Canvas object
     */
    std::shared_ptr<Canvas2D> create_canvas_2d(const std::string &canvas_id = "",
                                               double width = 400,
                                               double height = 400,
                                               const std::string &html_id = "",
     ...
     */
    std::shared_ptr<Graph> create_graph(const std::string &canvas_id = "",
                                        double width = 400,
                                        double height = 400,
                                        const std::string &html_id = "",
     ...
     *  \param audio_id a unique identifier for the AudioTrack (will be automatically populated if not provided).
     ...
     *  \param video_id a unique identifier for the Video (will be automatically populated if not provided).
     ...
     *  \param use_texture_alpha whether to use the alpha channel in the texture for transparency (only relevant for textured Meshes).
     *  \param is_billboard draw this Mesh as a billboard (i.e. always facing the user) rather than rotating with the rest of the world.
     *  \param is_label this mesh will be treated specially as a label. Not for public use.
     *  \return a reference to the new Mesh object
     */
    std::shared_ptr<Mesh> create_mesh(const std::string &mesh_id = "",
     ...
     *  Meshes are Scene-wide resources that can be reused across multiple Frames in multiple Canvas3Ds.
     *  This method can be used to create a new mesh which is simply a repositioning of an existing mesh,
     *  thus saving the cost of defining the entire topology every time the mesh is instantiated. The
     *  base mesh is not modified by this operation.
     ...
     *  This method can be used to create a new mesh which is simply a repositioning of an existing mesh,
     *  thus saving the cost of defining the entire topology every time the mesh is instantiated. The
     *  base mesh is not modified by this operation. Normals will be computed automatically from the
     *  provided positions.
     *  \param base_mesh_id the id of the base mesh to update
     *  \param positions the new vertex positions
     *  \param mesh_id a unique identifier for the Mesh (will be automatically populated if not provided).
     ...
     *  \param image_id a unique identifier for the Image (will be automatically populated if not provided).
     ...
     *  \param layer_id an identifier to allow several labels to be grouped together into a "layer".  Layers should be used in conjunction with Canvas3D.set_layer_settings().
     *  \param horizontal_align (one of 'left', 'center', 'right'): how to align the label horizontally.
     *  \param vertical_align (one of 'top', 'middle', 'bottom'): how to align the label vertically.
     *  \param offset_distance distance in world space to offset the label by when using left/right/top/bottom alignment.
     *  \param camera_space whether the label should be created in camera space
     *  \return a reference to the new Label object
     */
    std::shared_ptr<Label> create_label(const std::string &label_id = "",
     ...
     *  \param text_panel_id a unique identifier for the TextPanel (will be automatically populated if not provided).
     *  \param html_id optional id of an HTML element to use as this TextPanel's parent in the HTML DOM (otherwise will simply be appended to document).
     *  \param add_input_box whether to add an input box to the TextPanel
     *  \return a reference to the new TextPanel object
     */
    std::shared_ptr<TextPanel> create_text_panel(const std::string &text_panel_id = "",
                                                 const std::string &html_id = "",
                                                 bool add_input_box = false);

    /** Create a new DropDownMenu and add directly to the HTML page.
     *  \param drop_down_menu_id  a unique identifier for the DropDownMenu (will be automatically populated if not provided).
     *  \param title a title for the drop down box
     *  \param html_id optional id of an HTML element to use as this TextPanel's parent in the HTML DOM (otherwise will simply be appended to document).
     *  \return an reference to the new DropDownMenu object
     */
    std::shared_ptr<DropDownMenu> create_drop_down_menu(const std::string &drop_down_menu_id = "",
     ...
     *  \param path the path to the file on disk
     */
    void save_as_script(const std::string &path) const;
     ...
     *  \param relative_error_threshold the maximum expected error as a multiple of the range of values in the base mesh
     *  \param absolute_error_threshold the maximum expected error in absolute units
     *  \param base_mesh_id optional base mesh where quantization is applied. Otherwise all mesh updates are quantized.
     *  \param per_frame_range whether to use the most accurate range per update, increasing accuracy but reducing compression.
     *  \return the per-frame quantization information
     */
    std::map<std::string, QuantizationInfo> quantize_updates(float relative_error_threshold=1e-5,
...
private:
    template <typename T>
    static void add_commands(JsonValue &commands, const std::vector<std::shared_ptr<T>> &entities)
    {
