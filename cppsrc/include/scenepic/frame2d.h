...
public:
     ...
     */
    void add_line(const CoordinateBuffer &coordinates,
                  const Color &line_color = Colors::Black,
                  float line_width = 1.0f,
                  const Color &fill_color = Color::None(),
                  bool close_path = false, const std::string &layer_id = "");
     ...
     *  \param layer_id the unique ID of the layer for this primitive
     */
    void add_rectangle(float x, float y, float w, float h,
                       const Color &line_color = Colors::Black,
                       float line_width = 1.0f,
                       const Color &fill_color = Color::None(),
     ...
     *  \param smoothed whether to draw an image with smoothing or using nearest neighbor.
     *  \param layer_id the unique ID of the layer for this primitive
     */
    void add_image(const std::string &image_id,
                   const std::string &position_type = "fit",
                   float x = 0.0f, float y = 0.0f, float scale = 1.0f,
                   bool smoothed = false, const std::string &layer_id = "");

    /** Add a video to the frame. The image shown will be the corresponding frame from the video.
     *  \param position_type one of "fit", "fill", "stretch", or "manual"
     *  \param x x coordinate to draw the video frame at if position_type == "manual".
     *  \param y y coordinate to draw the video frame at if position_type == "manual".
     *  \param scale scale factor to draw the video frame by if position_type == "manual".
     *  \param smoothed whether to draw the video frame with smoothing or using nearest neighbor.
     *  \param layer_id the unique ID of the layer for this primitive
     */
    void add_video(const std::string &position_type = "fit",
                   float x = 0.0f, float y = 0.0f, float scale = 1.0f,
                   bool smoothed = false, const std::string &layer_id = "");
...
private:
    ...
    IndexVector lookup_layer_ids(const std::vector<std::string> &layer_ids) const;
    ...
    {
    public:
        Video(const std::string &position_type,
              std::uint32_t index,
              float scale,
              bool smoothed,
              const std::string &layer_id);
