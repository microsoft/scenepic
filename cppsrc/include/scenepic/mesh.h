// ----------------------------------------------------------------------------
//
// mesh.h -- Class providing the functionality for ScenePic 3D meshes
//
...
public:
     ...
     */
    void add_triangle(const Color &color = Color::None(),
     ...
     */
    void add_quad(const Color &color = Color::None(),
     ...
     * \param x_axis direction and length of the x-axis of the image in world space
     * \param y_axis direction and length of the y-axis of the image in world space
     * \param normal will be computed automatically if set to zero
     ...
     */
    void add_image(const Vector &origin = Vector(-0.5f, -0.5f, 0),
     ...
     * \param add_wireframe whether to add a wireframe outline
     */
    void add_cube(const Color &color = Color::None(),
                  const Transform &transform = Transform::Identity(),
                  bool fill_triangles = true,
                  bool add_wireframe = false);
     ...
     * \param add_wireframe whether to add a wireframe outline
     */
    void add_thickline(const Color &color = Color::None(),
     ...
     * \param add_wireframe whether to add a wireframe outline
     */
    void add_cone(const Color &color = Color::None(),
                  const Transform &transform = Transform::Identity(),
     ...
     * \param transform optional transform
     */
    void add_coordinate_axes(float length = 1.0f,
     ...
     * \param transform optional transform
     */
    void add_camera_frustum(const Color &color = Color::None(),
     ...
     * \param add_wireframe whether to add a wireframe outline
     */
    void add_disc(const Color &color = Color::None(),
                  const Transform &transform = Transform::Identity(),
     ...
     * \param add_wireframe whether to add a wireframe outline
     */
    void add_cylinder(const Color &color = Color::None(),
                      const Transform &transform = Transform::Identity(),
                      std::uint32_t segment_count = 10,
                      bool fill_triangles = true,
                      bool add_wireframe = false);
     ...
     * \param color required unless Mesh was constructed with shared_color argument.
     * \param transform optional transform
     * \param steps number of subdivision steps to apply to the icosahedron base primitive
     * \param fill_triangles whether to fill the primitive
     * \param add_wireframe whether to add a wireframe outline
     */
    ...
    /** Add a unit diameter UV sphere to this Mesh. If the mesh has a texture attached, this method
     *  will automatically create a set of vertex UVs which unwraps the sphere to map to the full
     *  UV space.
     * \param color required unless Mesh was constructed with shared_color argument.
     * \param transform optional transform
     * \param lat_count number of discrete samples in latitude
     ...
     * \param transform optional transform
     * \param reverse_triangle_order whether to reverse the triangle winding order
     * \param fill_triangles whether to fill the primitive
     * \param add_wireframe whether to add a wireframe outline
     */
    void add_mesh_without_normals(const ConstVectorBufferRef &vertices,
     ...
     *  \param reverse_triangle_order whether to reverse the triangle order when computing
     *                                the normals.
     *  \return a buffer containing the normalized per-vertex normals
     */
    static VectorBuffer compute_normals(const ConstVectorBufferRef &vertices,
                                        const ConstTriangleBufferRef &triangles,
                                        bool reverse_triangle_order = false);
     ...
     * \param add_wireframe whether to add a wireframe outline
     */
    void add_mesh_with_normals(const ConstVectorBufferRef &vertices,
     ...
     * \param add_wireframe whether to add a wireframe outline
     */
    void add_mesh(const std::shared_ptr<MeshInfo> &mesh_info,
                  const Transform &transform = Transform::Identity(),
                  bool reverse_triangle_order = false,
                  bool fill_triangles = true,
                  bool add_wireframe = false);
     ...
     * \param start_points defines N line start positions (and, optionally, the per-point colors)
     * \param end_points defines N line end positions (and, optionally, the per-point colors)
     * \param color if provided, will override any per-point colors stored in start_colors and end_colors.
     * \param transform optional transform
     */
    void add_lines(const ConstVertexBufferRef &start_points,
     ...
     * \param colors if provided, a per-instance color that overrides a Mesh's shared_color or per-vertex color
     * \param rotations if provided, a per-instance quaternion rotation that rotates each instance
     */
    void enable_instancing(const ConstVectorBufferRef &positions,
...
private:
    ...
    std::uint32_t append_vertex(const Vector &pos, const Vector &normal, const Color &color);
    std::uint32_t append_vertex(const Vector &pos, const Vector &normal, UV texture_uv);
    std::uint32_t append_vertex(const Vector &pos, const Vector &normal);
    void append_triangle(std::uint32_t index0, std::uint32_t index1, std::uint32_t index2);
