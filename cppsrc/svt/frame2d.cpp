...
void Frame2D::add_line(const CoordinateBuffer &coordinates,
                            ...
                            const std::string &layer_id)
{
    ...
    coordinates << x, y,
        ...
        x + w, y + h,
        x, y + h;
    this->add_line(coordinates, line_color, line_width, fill_color, true, layer_id);
}

void Frame2D::add_circle(float x, float y, float radius,
                         const Color &line_color,
                         float line_width,
                         const Color &fill_color,
                         const std::string &layer_id)
...
Frame2D::Text::Text(const std::string &text,
                    std::uint32_t index,
                    const Color &color,
                    float size_in_pixels,
                    const std::string &font_family,
                    const std::string &layer_id) : m_text(text),
                                                   m_index(index),
...
Frame2D::Image::Image(const std::string &image_id,
                      const std::string &position_type,
                      std::uint32_t index,
                      float scale,
                      bool smoothed,
                      const std::string &layer_id) : m_image_id(image_id),
...
Frame2D::Video::Video(const std::string &position_type,
                      std::uint32_t index,
                      float scale,
                      bool smoothed,
                      const std::string &layer_id) : m_position_type(position_type),
                                                     m_index(index),
                                                     m_scale(scale),
                                                     m_smoothed(smoothed),
                                                     m_layer_id(layer_id)
