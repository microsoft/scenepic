...
VertexList keys(const VertexMap &vertex_map)
{
    ...
    std::transform(vertex_map.begin(), vertex_map.end(), std::back_inserter(result),
...
struct SubdivisionBuilder
{
    ...
    Triangle::Scalar add_new_vertex(const VertexList &idxs, const std::vector<float> &weights)
    {
        auto new_index = this->v_indptr.size();
        this->v_indptr.push_back(static_cast<Triangle::Scalar>(this->v_weights.size()));
        this->v_weights.insert(this->v_weights.end(), weights.begin(), weights.end());
        this->v_generating_idxs.insert(this->v_generating_idxs.end(), idxs.begin(), idxs.end());
        return static_cast<Triangle::Scalar>(new_index);
    }

    void generate_vertex_vertex(Triangle::Scalar i_vertex, const VertexListMap &v_neighbors, int steps)
    {
        ...
        if (steps == 0)
        {
            ...
            std::fill(weights.begin() + 1, weights.end(),
    ...
    void add_triangle(Triangle::Scalar a, Triangle::Scalar b, Triangle::Scalar c)
    {
        this->triangles.emplace_back(this->v_idx[a], this->v_idx[b], this->v_idx[c]);
    }

    void add_triangles(const VertexListMap &v_neighbors, Triangle::Scalar a, Triangle::Scalar b, Triangle::Scalar c)
    {
        ...
        {
            this->triangles.emplace_back(this->v_idx[a], this->ev_idx[ab], this->ev_idx[ca]);
        }

        if (this->has_vertex(ab) && this->has_vertex(bc) && this->has_vertex(ca))
        {
            this->triangles.emplace_back(this->ev_idx[ab], this->ev_idx[bc], this->ev_idx[ca]);
        }

        if (v_neighbors.count(c))
        {
            this->triangles.emplace_back(this->ev_idx[ca], this->ev_idx[bc], this->v_idx[c]);
        }

        if (v_neighbors.count(b))
        {
            this->triangles.emplace_back(this->ev_idx[ab], this->v_idx[b], this->ev_idx[bc]);
        }
    }

    void generate_edge_vertex(const Edge &edge, const VertexMapList &v_1ringlinks)
    {
    ...
    LoopSubdivisionStencil build(std::size_t vertex_count)
    {
        ...
        Triangle::Scalar num_nonzero = static_cast<Triangle::Scalar>(this->v_weights.size());
...
LoopSubdivisionStencil LoopSubdivisionStencil::create(const ConstTriangleBufferRef triangles,
                                                        int steps, bool project_to_limit)
{
    assert(steps > 0 || (steps == 0 && project_to_limit));

    // Build an index that gives the opposite vertex in a triangle, given a vertex
    // and an outward pointing half-edge (encoded here using the index of the vertex
    ...
    for (auto row = 0; row < triangles.rows(); ++row)
    {
        ...
        for (auto i = 0; i < 3; ++i)
        {
            v_1ringlinks[tri_verts[i]][tri_verts[(i + 1) % 3]] = tri_verts[(i + 2) % 3];
        }
    }
    ...
    for (auto row = 0; row < triangles.rows(); ++row)
    {
        ...
        if (steps == 0)
        {
            if (v_neighbors.count(a) && v_neighbors.count(b) && v_neighbors.count(c))
            {
        ...
        for (auto &edge : edges(a, b, c))
        {
            ...
            if (v_neighbors.count(edge.first) == 0 && v_neighbors.count(edge.second) == 0)
            {
                continue;
            }
    ...
    {
        auto next_stencil = LoopSubdivisionStencil::create(stencil.m_triangles, steps - 1, project_to_limit);
        return LoopSubdivisionStencil(next_stencil.m_triangles, next_stencil.m_subdiv * stencil.m_subdiv);
    }
