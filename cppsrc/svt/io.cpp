...
Corner resolve_corner(const Corner &vertex,
                      ...
                      const std::vector<std::size_t> &uv_index)
{
    ...
    if (pos_idx < 0)
    {
        pos_idx = static_cast<std::int32_t>(position_index[position_index.size() + pos_idx]);
    }
    else if (pos_idx > 0)
    {
...
std::shared_ptr<MeshInfo> load_obj(std::istream &stream)
{
    ...
    cb.index_cb = [](void *user_data, tinyobj::index_t *indices, int num_indices) {
        ...
        if(num_indices == 3){
            for(auto idx=indices; idx < indices + 3; ++idx){
                mesh_data->corners.emplace_back(idx->vertex_index, idx->texcoord_index);
            }
            ...
            mesh_data->corners.emplace_back(indices->vertex_index, indices->texcoord_index);
        }else{
            throw std::invalid_argument("Only triangles and quad meshes are supported.");
        }
    };
    ...
    std::transform(mesh_data.corners.begin(), mesh_data.corners.end(), mesh_data.corners.begin(),
                   [&](const Corner &vertex) -> Corner { return resolve_corner(vertex, position_index, uv_index); });
    ...
    auto mesh_info = std::make_shared<MeshInfo>(num_vertices, num_triangles, has_uvs, false);
