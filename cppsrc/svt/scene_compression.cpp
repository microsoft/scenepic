...
namespace
{
    const float NUM_BINS = static_cast<float>(svt::MeshUpdate::QuantizationBinCount);
    ...
    float compute_representable_range(float relative_error_threshold, float absolute_error_threshold, float range)
    {
        ...
        else
        {
            ...
            {
                throw std::invalid_argument("One of the thresholds must be greater than 0.");
            }
        }
    }

    float estimate_size_ratio(std::size_t num_keyframes, std::size_t num_updates, bool per_frame_range)
    {
...
namespace scenepic
{
    ...
    {
        KeyframeAssignment(const std::shared_ptr<MeshUpdate> &frame, const std::shared_ptr<MeshUpdate> &keyframe) : frame_index(frame->frame_index()),
    ...
    std::vector<KeyframeAssignment> update_assignments(const std::vector<KeyframeAssignment> assignments,
                                                       ...
                                                       const std::shared_ptr<MeshUpdate> &keyframe)
    {
        ...
        {
            KeyframeAssignment new_assignment(updates[assignment.frame_index], keyframe);
    ...
    QuantizationInfo quantize_updates_for_mesh(float representable_range, std::vector<std::shared_ptr<MeshUpdate>> &updates, bool per_frame_range)
    {
        ...
        for (auto &assignment : assignments)
        {
            ...
            else
            {
                ...
                auto keyframe_vertex_buffer = updates[assignment.keyframe_index]->vertex_buffer();
                if(per_frame_range)
                {
                    frame_range = updates[assignment.frame_index]->difference_range(keyframe_vertex_buffer);
                }
                ...
                updates[assignment.frame_index]->quantize(assignment.keyframe_index,
                                                          frame_range,
                                                          keyframe_vertex_buffer);
            }
        }

        float mean_error = error_sum / static_cast<float>(updates.size() - num_keyframes);
        float estimated_size_ratio = estimate_size_ratio(num_keyframes, updates.size(), per_frame_range);

        return QuantizationInfo(num_keyframes, estimated_size_ratio, mean_error, max_error);
    }
    ...
    std::map<std::string, QuantizationInfo> Scene::quantize_updates(float relative_error_threshold, float absolute_error_threshold, const std::string &base_mesh_id, bool per_frame_range)
    {
        ...
        for (auto &mesh_updates : updates)
        {
            ...
            float representable_range = compute_representable_range(relative_error_threshold,
                                                                    absolute_error_threshold,
                                                                    mesh_range);
            info[mesh_updates.first] = quantize_updates_for_mesh(representable_range,
                                                                 mesh_updates.second,
                                                                 per_frame_range);
        }
