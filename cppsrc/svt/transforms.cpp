...
Transform rotation_matrix_from_axis_angle(const Vector &axis, float angle)
{
    ...
    Transform matrix;
    matrix << x * x + (1 - x * x) * cos, x * y * (1 - cos) - z * sin, x * z * (1 - cos) + y * sin, 0,
        x * y * (1 - cos) + z * sin, y * y + (1 - y * y) * cos, y * z * (1 - cos) - x * sin, 0,
        x * z * (1 - cos) - y * sin, z * y * (1 - cos) + x * sin, z * z + (1 - z * z) * cos, 0,
        0, 0, 0, 1;
    return matrix;
}
...
Quaternion quaternion_from_axis_angle(const Vector &axis, float angle)
{
    ...
    Quaternion quat(norm_axis(0) * half_sin, norm_axis(1) * half_sin, norm_axis(2) * half_sin, half_cos);
    return quat;
}
...
std::pair<Vector, float> axis_angle_to_align_x_to_axis(const Vector &axis)
{
    ...
    else
    {
        ...
        else
        {
            ...
            axis_angle.first /= std::sqrt(norm_axis(2) * norm_axis(2) + norm_axis(1) * norm_axis(1));
...
{
    auto axis_angle = axis_angle_to_align_x_to_axis(axis);
    return rotation_matrix_from_axis_angle(axis_angle.first, axis_angle.second);
}
...
Transform euler_angles_to_matrix(const Vector& euler_angles, const std::string &convention)
{
    Transform matrix = Transform::Identity();
    for(auto i=2; i >= 0; --i)
    {
...
Transform quaternion_to_matrix(const Quaternion& quaternion)
{
    ...
    float qz2 = qz * qz;
    Transform matrix;
    matrix << 1 - 2 * qy2 - 2 * qz2, 2 * qx * qy - 2 * qz * qw, 2 * qx * qz + 2 * qy * qw, 0,
...
Transform gl_projection(const Intrinsic& camera_matrix, int width, int height, double znear, double zfar)
{
    ...
    matrix << 2*K00/width, -2*K01/width, (width - 2*K02)/width, 0,
              0, 2*K11/height, (2*K12 - height)/height, 0,
              0, 0, A, B,
              0, 0, -1, 0;
    return matrix;
}
...
{
    Transform camera_to_world = extrinsic * rotation_about_x(static_cast<float>(M_PI));
