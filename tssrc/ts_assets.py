...
def _webvr_background(holo_deck_width=12.5,
    ...
    if line_color is None:
        line_color = sp.ColorFromBytes(255, 236, 100)
    ...
    # Create back wall
    wall = sp.Mesh(shared_color=line_color)
    ...
    holodeck.append_mesh(wall.get_transformed(sp.Transforms.RotationAboutXAxis(angle=np.pi)))
    holodeck.append_mesh(wall.get_transformed(sp.Transforms.RotationAboutYAxis(angle=-0.5*np.pi)))
    holodeck.append_mesh(wall.get_transformed(sp.Transforms.RotationAboutYAxis(angle=+0.5*np.pi)))
    holodeck.append_mesh(wall.get_transformed(sp.Transforms.RotationAboutXAxis(angle=-0.5*np.pi)))
    holodeck.append_mesh(wall.get_transformed(sp.Transforms.RotationAboutXAxis(angle=+0.5*np.pi)))
    holodeck.apply_transform(sp.Transforms.Scale(holo_deck_width))
    ...
    floor.apply_transform(sp.Transforms.Scale(holo_deck_width))
